...
void segtracker_add_seglist(const char *name, uae_u32 seglist_addr)
{
	if(seglist_addr == 0) {
		return;
	}
...
static uae_u32 REGPARAM2 LoadSeg(TrapContext *ctx)
{
	...
	segtracker_add_seglist(name, seglist_addr);
